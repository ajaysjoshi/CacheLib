...
void setupBlockCache(const navy::BlockCacheConfig& blockCacheConfig,
  ...
  //Align the metadata end to the zone size. So,the blockcache start
  //would be end of the metadata zones.
  if (usesZonedFile) {
    uint64_t totalMetaZone = metadataSize/regionSize;
    uint64_t adjustedBlockCacheOffset;
    if (metadataSize % regionSize)
      totalMetaZone++;
    //Block cache starts after the end of metadata zone
    adjustedBlockCacheOffset =
      alignUp(totalMetaZone * regionSize, regionSize);
    blockCacheOffset = adjustedBlockCacheOffset;
  }
